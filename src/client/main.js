        ...
        window.fetch = async (input, init) => {
            try {
                ...
                if ((path.startsWith('/api/ai/') || path === '/api/ai-unified/chat' || path === '/api/ai/agent') && wapi?.ai?.start) {
                    const payload = body?.provider ? body : { provider: 'openrouter', model: body?.model || 'openrouter/auto', system: body?.system || '', messages: body?.messages || [], apiKey: window?.__OPENROUTER_KEY__ || '' };
                    const session = await wapi.ai.start(payload);
                    if (session?.error)
                        return new Response(JSON.stringify({ error: session.error }), { status: 500, headers: { 'Content-Type': 'application/json' } });
                    // Return a ReadableStream Response that listens to chunk/done events
                    const stream = new ReadableStream({
                        start(controller) {
                            const enc = new TextEncoder();
                            const onChunk = (_e, m) => { if (m.sessionId !== session.sessionId || typeof m.text !== 'string')
                                return; controller.enqueue(enc.encode(`data: ${JSON.stringify({ choices: [{ delta: { content: m.text } }] })}\n\n`)); };
                            const onDone = () => { controller.close(); cleanup(); };
                            const onError = (_e, m) => { if (m.sessionId !== session.sessionId)
                                return; controller.error(new Error(m.error || 'AI error')); cleanup(); };
                            const off1 = wapi.ai.onChunk(session.sessionId, (t) => onChunk(null, { sessionId: session.sessionId, text: t }));
                            const off2 = wapi.ai.onDone(session.sessionId, () => onDone());
                            const off3 = wapi.ai.onError(session.sessionId, (err) => onError(null, { sessionId: session.sessionId, error: err }));
                ...
                return new Response(JSON.stringify({ error: e.message || 'IPC bridge failed' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
            }
        };
    }
}
