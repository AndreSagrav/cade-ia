...
githubRouter.post('/profile', async (req: Request, res: Response) => {
  const { token } = req.body;
  if (!token) return res.json({ ok: false, error: 'No GitHub token provided' });

  try {
    const resp = await fetch(`${GH_API}/user`, { headers: ghHeaders(token) });
    if (!resp.ok) return res.json({ ok: false, error: `GitHub API error: ${resp.status}` });
...
githubRouter.post('/repos', async (req: Request, res: Response) => {
  const { token, page = 1, per_page = 50 } = req.body;
  if (!token) return res.json({ ok: false, error: 'No GitHub token provided' });

  try {
    const resp = await fetch(
      `${GH_API}/user/repos?sort=updated&per_page=${per_page}&page=${page}&affiliation=owner`,
...
githubRouter.post('/create-repo', async (req: Request, res: Response) => {
  const { token, name, description = '', isPrivate = false } = req.body;
  if (!token) return res.json({ ok: false, error: 'No GitHub token provided' });
  if (!name) return res.status(400).json({ ok: false, error: 'Missing repo name' });
