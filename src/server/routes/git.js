...
gitRouter.post('/status', async (req, res) => {
    const { cwd } = req.body;
...
gitRouter.post('/log', async (req, res) => {
...
gitRouter.post('/diff', async (req, res) => {
...
gitRouter.post('/clone', async (req, res) => {
    ...
    if (!url || !destination)
        return res.status(400).json({ error: 'Faltan parámetros url o destination' });
    try {
...
gitRouter.post('/remote/list', async (req, res) => {
    const { cwd } = req.body;
    if (!cwd || !isGitRepo(cwd))
        return res.json({ ok: false, error: 'Not a git repository' });
...
gitRouter.post('/remote/add', async (req, res) => {
